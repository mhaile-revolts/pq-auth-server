...
1. Build the daemon from the repo root:
   ```bash
   mkdir -p build
   cd build
   cmake ..
...
2. Start `pq_authd` (typically on a Linux system where `/run` and `/var/log/pq-auth` exist):
   ```bash
   ./pq_authd
   ```

3. From another shell, send an AS request over the UNIX socket using `socat`:
   ```bash
...
4. Capture the `tgt` from that response and request a service ticket:
   ```bash
   TGT="<paste_tgt_here>"
...
5. Inspect `/var/log/pq-auth/auth.log` to verify that each raw request was logged by `AuditLogger`.
